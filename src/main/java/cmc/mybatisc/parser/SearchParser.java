...
public class SearchParser {
    ...
    private void parse(Search search, Method method, MapperParser mapperParser) {
        ...
        if (!StringUtils.hasText(this.table)) {
            this.table = mapperParser.getTableName();
        }
        if (!StringUtils.hasText(this.table)) {
            throw new RuntimeException("Table Name not found");
        }
        this.alias.setMainTable(this.table);
        // 设置表别名
    ...
    public static SearchParser parse(QueryFieldCriteria info) {
        ...
        searchParser.delFlag = new DelFlag[]{};
        ...
        if (!StringUtils.hasText(searchParser.table)) {
            searchParser.table = info.getMapperParser().getTableName();
        }
        if (!StringUtils.hasText(searchParser.table)) {
            throw new RuntimeException("Table Name not found");
        }
        searchParser.alias.setMainTable(searchParser.table);
        // 设置表别名
     ...
     * 此函数需要优化，有些语义不明了
     *
     * @param delFlags del标志
     * @return {@link DelFlag}
     */
    public DelFlag getDelFlag(DelFlag[] delFlags) {
        if (this.mapperParser.getMapperStrong() != null) {
            delFlags = this.mapperParser.getMapperStrong().delFlag();
        }
        for (DelFlag delFlag : delFlags) {
